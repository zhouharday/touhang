    ...
    <div class="form">
        ...
        <el-table :data="formAccountInfo" style="width: 100%">
            ...
            <el-table-column label="开户行">
                <template scope="scope">
                    <el-autocomplete class="inline-input" v-model="scope.row.openingBank" :fetch-suggestions="querySearch" placeholder="请输入内容" @select="handleSelect" style="width:100%;" :disabled="scope.row.flag">
                    </el-autocomplete>
                </template>
            </el-table-column>
            <el-table-column label="账号">
                <template scope="scope">
                    <el-input v-model="scope.row.accountNumber" placeholder="请输入内容" :disabled="scope.row.flag">
                    </el-input>
                </template>
            </el-table-column>
        </el-table>
...
>>>>>>> 4e160a790bf5d4a0ce1fd8b5d27d5285efde6439
  ...
  mounted() {
    ...
    this.managementType =
      JSON.parse(sessionStorage.getItem("MANTYPE")) || this.getManType;
    this.OrgTypeList =
      JSON.parse(sessionStorage.getItem("ORGTYPE")) || this.OrgType;
  },
  computed: {
    // loading: function(){
    ...
    ...mapGetters(["getManType", "OrgType", "fundInvestment", "getOrgList"])
  },
  created() {
